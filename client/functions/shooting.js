...
function mouseCoordsGet() {
  player.lastMouseX =
    mousePos.x - canvas.offsetLeft + player.pos.x - 0.5 * canvasDiv.clientWidth;
  player.lastMouseY =
    mousePos.y - canvas.offsetTop + player.pos.y - 0.5 * canvasDiv.clientHeight;
}
...
function shootingCheck(shoot) {
  if (shoot === false) {
    ...
    trailWidth = 3;
    player.theta +=
      Math.random() * (player.weapon.spray + player.weapon.spray) -
      player.weapon.spray;
  }
  ...
  if (player.weapon != player.thirdWeapon) {
    obstacles.forEach(function (obs) {
      ...
      let angleLU = Math.atan2(
        obs.lu[1] - player.pos.y,
        obs.lu[0] - player.pos.x
      );
      let angleRU = Math.atan2(
        obs.ru[1] - player.pos.y,
        obs.ru[0] - player.pos.x
      );
      let angleLD = Math.atan2(
        obs.ld[1] - player.pos.y,
        obs.ld[0] - player.pos.x
      );
      let angleRD = Math.atan2(
        obs.rd[1] - player.pos.y,
        obs.rd[0] - player.pos.x
      );
      //this huge IF checks in which point trajectory crosses the obs
      //if obs is on the right side of player
      if (obs.x > player.pos.x) {
        if (obs.y + obs.height < player.pos.y) {
          //if obs is above player
          //in this case the biggest angle is lu and rd, the LD is in the middles
          if (
            player.theta >= angleLU &&
            player.theta <= angleRD &&
            player.theta <= angleLD
          ) {
            //left of obs
            currentClosePoints.push(
              (obs.x - player.pos.x) ** 2 +
                (-coefficient * (obs.x - player.pos.x)) ** 2
            );
          } else if (
            player.theta >= angleLU &&
            player.theta <= angleRD &&
            ...
            //down
            currentClosePoints.push(
              (-(obs.y + obs.height - player.pos.y) / coefficient) ** 2 +
                (obs.y + obs.height - player.pos.y) ** 2
            );
          }
        } else if (obs.y > player.pos.y) {
          //if it's under player
          if (
            player.theta >= angleRU &&
            player.theta <= angleLD &&
            player.theta >= angleLU
          ) {
            //left
            ...
            //up
            currentClosePoints.push(
              (-(obs.y - player.pos.y) / coefficient) ** 2 +
                (obs.y - player.pos.y) ** 2
            );
          }
        } else if (
          obs.y + obs.height >= player.pos.y &&
          obs.y <= player.pos.y
        ) {
          //if obs is on the y = player.pos.y line
          if (player.theta >= angleLU && player.theta <= angleLD) {
            //left
            currentClosePoints.push(
              (obs.x - player.pos.x) ** 2 +
                (-coefficient * (obs.x - player.pos.x)) ** 2
      ...
      //if obs is on the left side of player
        ...
        if (obs.y + obs.height < player.pos.y) {
          ...
          if (
            ...
            //right
            currentClosePoints.push(
              (-player.pos.x + obs.x + obs.width) ** 2 +
                (coefficient * (player.pos.x - obs.x - obs.width)) ** 2
            );
          }
        } else if (obs.y > player.pos.y) {
          //if it's under player
          ...
          //if obs is on the y = player.pos.y line
          if (player.theta >= angleRD || player.theta <= angleRU) {
            //right
            currentClosePoints.push(
              (-player.pos.x + obs.x + obs.width) ** 2 +
                (coefficient * (player.pos.x - obs.x - obs.width)) ** 2
      ...
      //if obs is in the same x line with player
        ...
        if (obs.y + obs.height < player.pos.y) {
          //above
          if (player.theta == (1 / 2) * (0 - Math.PI)) {
            //if k --> -OO
            currentClosePoints.push((player.pos.y - obs.y - obs.height) ** 2);
          }
          if (player.theta >= angleLD && player.theta <= angleRD) {
            //else, i.e. down
            currentClosePoints.push(
              (-(obs.y + obs.height - player.pos.y) / coefficient) ** 2 +
                (obs.y + obs.height - player.pos.y) ** 2
          ...
          //under
          if (player.theta == (1 / 2) * Math.PI) {
            //if k --> OO
            currentClosePoints.push((obs.y - player.pos.y) ** 2);
          }
          if (player.theta >= angleRU && player.theta <= angleLU) {
            //else, i.e. up
            currentClosePoints.push(
              (-(obs.y - player.pos.y) / coefficient) ** 2 +
                (obs.y - player.pos.y) ** 2
  ...
  players.forEach(function (enemy) {
    ...
    let distance =
      ((enemy.pos.y - player.pos.y) ** 2 + (enemy.pos.x - player.pos.x) ** 2) **
      0.5;
    ...
    if (enemy.pos.x != player.pos.x || enemy.pos.y != player.pos.y) {
      ...
      //if the bigger angle is more than PI radians
        ...
        if (
          player.theta >= angleSmall ||
          player.theta <= angleLarge - 2 * Math.PI
        ) {
          currentClosePoints.push((Math.sqrt(distance ** 2 - b ** 2) - a) ** 2);
        }
      }
      //if the smaller angle is less than -PI radians
        ...
        if (
          player.theta <= angleLarge ||
          player.theta >= angleSmall + 2 * Math.PI
        ) {
          currentClosePoints.push((Math.sqrt(distance ** 2 - b ** 2) - a) ** 2);
        }
      }
    ...
    //then it can get shoot, so push it into the "main" list
    if (
      currentClosePoints[0] <= player.weapon.shootingDist ** 2 &&
      currentClosePoints[0] >= player.weapon.shootingMinDist ** 2
    ) {
      closePList.push(currentClosePoints[0]);
      ...
      if (
        Math.sqrt(
          (enemy.pos.x - player.pos.x - eX) ** 2 +
            (enemy.pos.y - player.pos.y - eY) ** 2
        ) <= player.weapon.damageRadius
      ) {
        if (shoot && enemy.health > 0) {
          shootSendHit(
            player.pos.x,
            player.pos.y,
            player.pos.x + Math.cos(player.theta) * closePList[0] ** 0.5,
            player.pos.y + Math.sin(player.theta) * closePList[0] ** 0.5,
            enemy.id,
  ...
  players.forEach(function (cplayer) {
    if (
      cplayer.crossPointDistance === closePList[0] &&
      shoot === true &&
      cplayer.health > 0
    ) {
